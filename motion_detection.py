...
def motion_detection():
    ...
    while True:
        ...
        for contour in cntr:
            if cv2.contourArea(contour) > 800: # if contour area is less then 800 non-zero(not-black) pixels(white)
                (x, y, w, h) = cv2.boundingRect(contour) # x,y are the top left of the contour and w,h are the width and hieght 
